class InspectionFile < ApplicationRecord
  ...
  has_one :preview, class_name: 'InspectionFilePreview', dependent: :destroy

  # Delegations
  delegate :site, to: :inspection
  ...
  scope :grouped_by_scan_series, -> { group_by(&:scan_series) }
  ...
  def match_scan_protocol(force=false)
    if (file_data_changed? || force) && file_attacher.stored?
      inspection.site.scan_protocols.matching(scan_header).most_specific.first.tap do |protocol|
